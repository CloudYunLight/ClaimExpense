...
class DatabaseUtil {
  ...
  static async initializeAdminUser() {
    try {
      ...
      if (userCount === 0) {
        // 表为空，创建初始管理员用户；优先读取环境变量口令，避免固定弱口令。
        const adminUsername = process.env.INIT_ADMIN_USERNAME || 'admin';
        const bootstrapPassword = process.env.INIT_ADMIN_PASSWORD || crypto.randomBytes(8).toString('hex');
        const passwordGenerated = !process.env.INIT_ADMIN_PASSWORD;
        const hashedPassword = await bcrypt.hash(bootstrapPassword, parseInt(config.bcrypt.saltRounds));
        
        await this.execute(
          'INSERT INTO users (username, password, real_name, role, status) VALUES (?, ?, ?, ?, ?)',
          [adminUsername, hashedPassword, 'Administrator_default', 1, 1]
        );

        logger.info('初始管理员用户已创建，请尽快修改密码', {
          username: adminUsername,
          passwordFromEnv: !passwordGenerated
        });

        if (passwordGenerated) {
          console.warn(`[SECURITY] Bootstrap admin created with generated password. username=${adminUsername}, password=${bootstrapPassword}`);
          console.warn('[SECURITY] Please sign in once and change this password immediately.');
        }
